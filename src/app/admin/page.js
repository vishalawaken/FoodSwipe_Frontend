...
const AdminPage = () => {
    ...
    const fetchPendingRestaurants = async () => {
        try {
            setLoading(true);
            const res = await axios.get(`${process.env.NEXT_PUBLIC_API_URL}/api/restaurants/pendingRestaurants`, { withCredentials: true })
    ...
    const approveRestaurant = async (restaurantId) => {
        try {
            await axios.patch(`${process.env.NEXT_PUBLIC_API_URL}/api/restaurants/${restaurantId}/approve`,
                {},
